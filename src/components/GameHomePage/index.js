...
// const initialRandomNumber = Math.floor(Math.random() * imagesList.length)
...
class GameHomePage extends Component {
  ...
  getRandomImage = () => {
    ...
    const randomImage = imagesList[random]

    //  random === '' ? imagesList[initialRandomNumber] :
    // console.log(randomImage)

    return (
      <img className="random-image" src={randomImage.imageUrl} alt="match" />
    )
  }

  //   getNewRandomImage = r => {
  //     const randomImage = imagesList[r]
  //     // console.log(randomImage)
  ...
  onClickImage = id => {
    ...
    if (
      //   imagesList[initialRandomNumber].id === id ||
      imagesList[random].id === id
    ) {
      //   console.log(true)
      //   console.log(imagesList[initialRandomNumber].id)
      //   console.log(imagesList[random])

      //   const randomN = Math.floor(Math.random() * imagesList.length)
  ...
  getGameResult = () => {
    ...
    return (
      <div className="game-result-container">
        ...
        />
        <h3 className="score-text">YOUR SCORE</h3>
  ...
  render() {
    ...
    // console.log(imageList[initialRandomNumber])

    const {activeTab, score, time, out} = this.state

    // console.log(this.state)

    // console.log(filteredList[randomNumber])
        ...
        <div className="game-container">
          {out ? (
            ...
            <div className="game-image-container">
              {/* <img src={randomImage.imageUrl} alt={randomImage.id} /> */}
              {this.getRandomImage()}
              <ul className="tab-items-list">
                {tabsList.map(eachTab => (
                  <TabItem
                    tabDetails={eachTab}
                    key={eachTab.tabId}
                    changeTabItem={this.onChangeTabItem}
                    isActive={activeTab === eachTab.tabId}
                  />
                ))}
              </ul>

              <ul className="images-items-list">
                {filteredList.map(eachImage => (
                  <GameItem
                    imageDetails={eachImage}
                    key={`thumbnail${eachImage.id}`}
